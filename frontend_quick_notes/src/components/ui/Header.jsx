import React from "react";
import Button from "./Button";
import { useNotes } from "../../store/NotesContext";
import { useTheme } from "../../theme/theme";

// PUBLIC_INTERFACE
export default function ThemedHeader({ onSave }) {
  /** Themed Header that mirrors original Header.js functionality and includes a ThemeSwitcher. */
  const { state, actions } = useNotes();
  const { themeKey, setTheme } = useTheme();
  const active = state.notes.find((n) => n.id === state.activeId) || null;

  const nextThemeKey = themeKey === "ocean" ? "quicknote" : "ocean";
  const switchLabel = themeKey === "ocean" ? "Quick Note Community" : "Ocean Professional";

  return (
    <header className="header" role="banner">
      <h1 className="header-title" aria-label="Application title">Quick Notes</h1>

      <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
        {/* Theme Switcher - minimal inline control */}
        <Button
          aria-label={`Switch theme to ${switchLabel}`}
          title={`Theme: ${themeKey === "ocean" ? "Ocean Professional" : "Quick Note Community"}`}
          onClick={() => setTheme(nextThemeKey)}
        >
          Theme: {themeKey === "ocean" ? "Ocean" : "QNC"}
        </Button>

        <div style={{ marginLeft: 8, marginRight: 8, fontSize: 12, opacity: 0.9 }}>
          {state.authUser ? `Signed in as ${state.authUser.name || state.authUser.email || state.authUser.id}` : ""}
        </div>
        <div className="header-actions" role="toolbar" aria-label="Note actions">
          <Button
            variant="primary"
            onClick={() => actions.createNote()}
            aria-label="Create a new note (Ctrl or Command and N)"
          >
            + New
          </Button>
          <Button
            onClick={() => active && onSave?.()}
            disabled={!active}
            aria-disabled={!active}
            aria-label="Save current note (Ctrl or Command and S)"
          >
            Save
          </Button>
          <Button
            onClick={() => active && actions.deleteNote(active.id)}
            disabled={!active}
            aria-disabled={!active}
            aria-label="Delete current note"
          >
            Delete
          </Button>
          {state.authUser && (
            <Button
              onClick={() => actions.logout()}
              aria-label="Sign out"
              title="Sign out"
            >
              Logout
            </Button>
          )}
        </div>
      </div>
    </header>
  );
}
